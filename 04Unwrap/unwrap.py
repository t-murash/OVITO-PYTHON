...
def modify(frame,data):
    ...
    ipbc=data.particles['Periodic Image']
    ...
    for i in range(num_particles):
        pid_i=pid[i]
        pbc_sort[pid_i-1,0]=ipbc[i,0]
        pbc_sort[pid_i-1,1]=ipbc[i,1]
        pbc_sort[pid_i-1,2]=ipbc[i,2]

    #for i in range(mol_max):
    #    print(i,mol_id_min[i],mol_id_max[i])
    ...
    #    mol_i=mol[i]
    #    if mol_i == 40:
    #        print(ipbc[i],pid[i],pbc_mid[mol_i-1])
...
vp = Viewport(type=Viewport.Type.Perspective, camera_dir=(1,1,-1),camera_pos=(-2*lx,-2*ly,3.0*lz))
#vp.zoom_all()
